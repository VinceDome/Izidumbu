...
class Move:
    ...
    def g(self, speed, rot, initial_deg=False, multiplier=2, givenBrake=True, motorsEnd = False, rampup=[0, 0], rampdown=[0, 0], timeout=10000):
...
class Runs:
    ...
    def run1(self):
        ...
        #kitolja
        self.move.g(-80, 1280, initial_deg=1, rampup = [0, 0.5], rampdown=[0, 300], multiplier=2)
        self.move.g(40, 150+100, initial_deg=1, rampdown=[0, 50], multiplier=2.3)
        ...
        self.move.g(40, 350, initial_deg=-50, rampup=[0, 0.5], rampdown=[0, 100], multiplier=2.3)
        ...
        self.move.t(-50, exponent=1.1, timeout=2)
        self.move.g(-30, 250-50, initial_deg=-45, rampup=[0, 0.5], rampdown=[0, 50], multiplier=2.5)
        self.move.t(-90)
        self.move.g(-30, 300, initial_deg=-90, timeout = 1.5, rampup=[0, 0.5], multiplier=2.5)
        ...
        self.move.g(30, 160+15, initial_deg=0, rampdown=[0, 80], multiplier=2.5)
        self.move.t(-90, motors="d")


        self.util.lever.on_for_degrees(-100, 100, block = False)

        self.move.g(30, 310+15, initial_deg=-90, rampup=[0, 0.5], rampdown=[0, 100], multiplier=2.8)
        ...
        #"""
        self.move.g(-20, 40, initial_deg=-140, multiplier=2.8)

        self.move.t(-90)

        self.move.g(15, 130, initial_deg=-90, multiplier=2.8)

        self.util.lever.on_for_degrees(-100, 800)
        self.time.sleep(0.5)
        self.util.lever.on_for_degrees(100, 400)

        self.util.lever.on_for_degrees(-40, 1000, block=False)
        self.move.g(-30, 300, initial_deg=-90)


        self.move.g(60, 1100, initial_deg=-78, motorsEnd=True, rampup=[0, 0.5], multiplier=2)

        self.move.g(60, 550, initial_deg=-95, motorsEnd=True, multiplier=2)
        self.util.lever.on_for_degrees(100, 1000, block=False)
        self.move.g(70, 950, initial_deg=-95, rampup=[60, 0.5], multiplier=1.5)
        #"""
    ...
    def run2(self):
        ...
        self.move.g(-60, 500, initial_deg=-260, rampdown=[0, 150], multiplier=1.2)
    ...
    def run3(self):
        ...
        self.move.g(100, 2600-2100, initial_deg=20, rampup=[0, 0.5], multiplier=0.8, motorsEnd=True)
        self.move.g(100, 2100, initial_deg=24, givenBrake=False, multiplier=0.9)
    ...
    def run4(self):
        ...
        #! új feltét teszt
        self.move.g(70, 150, initial_deg=0, rampup=[30, 0.5], motorsEnd=True, multiplier=1.5)
        self.move.g(70, 1550, initial_deg=7, rampdown = [0, 300], givenBrake=False, multiplier=1.5)

        self.util.lever.off()

        self.move.t(-20, motors="a", motorsEnd=True, timeout=3, tolerance=2, exponent=1.3)
        self.move.g(40, 100, initial_deg=-20, motorsEnd=True)

        self.move.t(-60, motors="a", givenBrake=False, motorsEnd=True, exponent=1.3, tolerance=2)

        self.move.g(40, 100, initial_deg=-66, rampup=[0, 0.5], motorsEnd=True)

        self.move.g(40, 80, initial_deg=-73, multiplier=2.5, rampdown=[0, 40])

        self.move.t(-120+10, motors="a", exponent=1.3, tolerance=2, timeout=3)

        #előremegy, aztán hirtelen vissza, és ráfordul az egyenesre
        self.move.g(60, 400, initial_deg=-110, motorsEnd=True, rampup=[0, 0.6])
        self.move.g(60, 360, initial_deg=-135, multiplier=1.5)
        #self.util.lever.on_for_degrees(100, 350, block=False)

        self.move.g(-60, 200+40, initial_deg=-125, motorsEnd=True, multiplier=1.5)
        self.move.g(-60, 735, initial_deg=-28+12, multiplier=0.4, rampdown=[0, 160])
        self.move.t(-239)
        #self.util.lever.on_for_degrees(-100, 350, block=False)
        #? batman feltét wonky..? start
        """
        #előremegy
        self.move.g(70, 100, initial_deg=0, rampup=[30, 0.5], motorsEnd=True, multiplier=1.5)
        self.move.g(70, 1650, initial_deg=8, rampdown = [0, 300], givenBrake=False, multiplier=1.5)
        ...
        self.move.g(40, 150, initial_deg=-60-6, rampup=[0, 0.5], rampdown=[0, 50])

        self.move.t(-80, tolerance=7, exponent=1.1, motors="a")

        self.move.g(30, 50, initial_deg=-73)

        self.move.t(-120, motors="a", exponent=1.3, tolerance=2, timeout=3)
        """
        ...
        self.move.g(60, 300+30, initial_deg=-135, multiplier=1.5)
        self.util.lever.on_for_degrees(100, 350, block=False)

        self.move.g(-60, 200, initial_deg=-120-5, motorsEnd=True, multiplier=1.5)
        self.move.g(-60, 735, initial_deg=-28, multiplier=0.4, rampdown=[0, 200])
        self.move.t(-239)
        self.util.lever.on_for_degrees(-100, 350, block=False)
        ...
        self.util.lever.on_for_degrees(-100, 1000, block=False)
        #elmegy a missionhöz
        self.move.g(80, 490-30-15, initial_deg=-239, rampup=[0, 0.6], rampdown=[20, 100], multiplier=1.9, motorsEnd=True)
        #self.move.g(7, 130, initial_deg=-239, multiplier=3)

        #self.move.g(-10, 35, initial_deg=-239, multiplier=3)
        #self.move.t(-239, exponent=1.3, timeout=1)
        ...
        for i in range(3):
            ...
            while self.util.lever.position > deg +5:
                ...
                if self.util.lever.is_stalled:
                    ...
                    self.util.lever.off()
                    self.time.sleep(0.1)
                elif time.perf_counter() > tic + 1.5:
                    break
            self.util.lever.off()

        self.util.lever.on_for_degrees(100, 200)
        self.move.g(-15, 18, initial_deg=-239, multiplier=2.8)
        #self.move.g(15, 30, initial_deg=-239, multiplier=2.8)

        self.move.t(-255, motors="d", exponent=1.2, tolerance=2, timeout=1.5)
        

        #!!!
        self.move.drive.off(brake=False)

        self.util.lever.on_for_degrees(100, 250-200, block=False)


        self.move.t(-330, motors="a", exponent=1.2, timeout=2, tolerance=1)
        self.move.g(10, 70, initial_deg=-330, timeout=1.5, multiplier=2.5)
        self.move.g(-10, 20, initial_deg=-330, timeout=1.5, multiplier=2.5)
        ...
        #self.move.g(10, 20, initial_deg=-315-15)

        self.util.lever.on_for_degrees(-100, 110)

        self.move.g(-20, 200+20, motorsEnd=True, rampup=[0, 0.5], initial_deg=-315-15)

        self.move.g(-80, 1300, initial_deg=-375+5, rampup=[-20, 1], multiplier=0.8)
    ...
    def run5(self):
        ...
        self.move.g(60, 200, initial_deg=0, rampup=[0, 1], motorsEnd=True, multiplier=1.5)
        self.util.lever.on_for_degrees(100, 300, block=False)
        self.move.g(60, 570-20, initial_deg=10-8, rampdown=[0, 140], multiplier=1.5)
        
        self.util.lever.on_for_degrees(-100, 180)

        self.move.g(-70, 500, initial_deg=10-8, rampup=[0, 0.6], motorsEnd=True, multiplier=1.5)

        self.util.lever.on(100)

        self.move.g(-70, 150, initial_deg=10-8, givenBrake=False)
    ...
    def run6(self):
        ...
        self.move.g(80, 1100+30, initial_deg=0, rampup=[0, 0.5], rampdown = [0, 250+50], multiplier=1.8)
        
        self.util.topping.off(brake=False)

        self.move.t(32, exponent=0.9)
       
        self.move.g(60, 480-15, initial_deg = 32, rampup = [0, 0.5], rampdown=[0, 300], timeout=2.2, multiplier=2)
        ...
        while self.time.perf_counter() < tic + 1.5:
        ...
        self.move.g(-55, 370+15+7, initial_deg=32, rampup=[0, 0.5], rampdown=[0, 150])

        #turn to 'The Hand'
        self.move.t(-23)

        """
        self.move.g(80, 870-20, initial_deg=-23-1, rampup=[0, 0.6], rampdown=[60, 170], motorsEnd=True)
        self.move.g(60, 200, initial_deg=-5, rampdown=[0, 80])
        """

        self.util.lever.on_for_degrees(-100, 100, block=False)
        self.move.g(80, 1035+40, initial_deg=-23, rampup=[0, 0.6], rampdown=[0, 300], multiplier=1.3, timeout=3)
        self.move.g(-20, 40, initial_deg=-23, rampup=[0, 0.4], rampdown=[0, 20])

        #raise 'The Hand'
        self.util.lever.on_for_degrees(30, 150, block=False)
        self.move.t(-2, exponent=0.9+0.05, timeout=2, tolerance=2)

        self.move.time.sleep(0.5)
        

        self.move.g(-40, 150-35, initial_deg=0, rampup=[0, 0.5], rampdown=[0, 70])

        #falaz
        self.move.t(-150, exponent=0.8-0.1, tolerance=1)
        self.util.lever.on_for_degrees(50, 400, block=False)
        self.move.g(-50, 250, initial_deg=-150, rampup=[0, 0.5], timeout=1.5)
        self.move.gyro.reset()
        self.move.time.sleep(0.5)

        self.util.lever.on_for_degrees(-100, 500, block=False)

        self.move.g(30, 200, rampup=[0, 0.4], initial_deg=12-1, rampdown=[0, 70])

        self.move.t(-7, exponent=1.2, motors="a")
        self.move.t(-7, exponent=1, motors="a")
        self.move.g(20, 100, initial_deg=-8, rampup=[0, 0.5], multiplier=3)
        self.util.lever.on_for_degrees(70, 700-100, block=False)

        self.move.g(30, 250, initial_deg=-25, rampup=[0, 0.5])
        self.move.t(-25, timeout=1.5, tolerance=1, exponent=1.1)

        self.util.lever.on_for_degrees(-50, 420+25)
        self.time.sleep(0.5)

        self.move.g(-40, 100-30, motorsEnd=True)
        self.move.g(-40, 600, initial_deg=-1, multiplier = 0.3+0.9, timeout=2.5)

        self.move.gyro.reset()
        self.move.time.sleep(0.5)

        self.move.g(40, 100, initial_deg=0, motorsEnd=True, rampup=[0, 0.5])
        self.move.g(40, 700+20, initial_deg=160, multiplier=0.28, motorsEnd=True)
        self.util.lever.on_for_degrees(-100, 1200, block=False)

        self.move.t(153+5)
        self.time.sleep(0.2)
        self.move.t(153+5, exponent=1.2)

        self.move.g(40, 160, initial_deg=153+5, rampup=[10, 0.5], multiplier=2.5)

        self.move.t(153+5, exponent=1.4)
        """
        #!old, wonky?
        self.move.g(40, 100, initial_deg=0, motorsEnd=True, rampup=[0, 0.5])
        ...
        self.move.t(148-2)
        self.time.sleep(0.3)
        self.move.t(148-2, exponent=1.2)

        self.move.g(40, 50+30, initial_deg=148-2, rampup=[10, 0.5], multiplier=2.5)

        self.move.t(148-2, exponent=1.4)
        """
        self.util.lever.off()
        self.util.lever.on_for_degrees(100, 450-100)
        self.util.lever.on_for_degrees(100, 800, block=False)
        ...
        """
        self.move.t(130, tolerance=10)
        self.move.t(160, tolerance=10)
...
class Menu:
    ...
    def selection(self, default):
        ...
        while True:
            ...
            #selection
            if self.button.right and not self.button.down:
            ...
            if self.button.left and not self.button.down:
                if selected == 1:
            ...
            #motorwiggle and motorcontrol launching
                ...
                if self.button.right or self.button.left:
                    self.util.topping.off(brake=False)
                    self.util.lever.off(brake=False)
                    motorcontrol = True
                    break
